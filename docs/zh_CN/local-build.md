...
当前本地编译使用官方 `zmkfirmware/zmk` checkout。

Sweep-Pro 的 keymap 是所有硬件版本共用的一份 `config/sweep.keymap`。触控板作为可选 shield 组合进构建，用户只需要按自己的硬件版本选择对应 UF2。
